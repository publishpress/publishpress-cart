...
class Style
{
    ...
    public function fromStyleSheets(AbstractTag $tag, $attributes) {
        ...
        $outputFormat = \PublishPress\Sabberworm\CSS\OutputFormat::createCompact();

        $styles = array();

        foreach ($stylesheets as $_sc) {

            /** @var \PublishPress\Sabberworm\CSS\RuleSet\DeclarationBlock $_decl */
            foreach ($_sc->getAllDeclarationBlocks() as $_decl) {

                /** @var \PublishPress\Sabberworm\CSS\Property\Selector $_selector */
                foreach ($_decl->getSelectors() as $_selector) {
                    ...
                    if ($class !== null) {
                        foreach ($class as $_class) {
                            if ($_selector === ".$_class") {
                                /** @var \PublishPress\Sabberworm\CSS\Rule\Rule $_rule */
                                foreach ($_decl->getRules() as $_rule) {
                                    $value = $_rule->getValue();
                                    if ($value instanceof \PublishPress\Sabberworm\CSS\Value\Value) {
