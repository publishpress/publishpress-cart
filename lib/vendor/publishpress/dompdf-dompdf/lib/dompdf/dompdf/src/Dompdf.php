 ...
 */
namespace PublishPress\Dompdf;

use DOMDocument;
use DOMNode;
use PublishPress\Dompdf\Adapter\CPDF;
use DOMXPath;
use PublishPress\Dompdf\Frame\Factory;
use PublishPress\Dompdf\Frame\FrameTree;
use PublishPress\Dompdf\Image\Cache;
use PublishPress\Dompdf\Css\Stylesheet;
use PublishPress\Dompdf\Helpers;
use PublishPress\Masterminds\HTML5;
 ...
 * the {@link PublishPress\Dompdf\Adapter\CPDF} class provides a simple, stateless interface to
 * PDF generation.)  PDFLib support has now also been added, via the {@link
 * PublishPress\Dompdf\Adapter\PDFLib}.
 *
 *
 * @package dompdf
 */
class Dompdf
{
    ...
    */
    ...
    public function render()
    {
        ...
        if (
            \PublishPress\Dompdf\Helpers::lengthEqual($canvasWidth, $size[2]) === false ||
            \PublishPress\Dompdf\Helpers::lengthEqual($canvasHeight, $size[3]) === false
        ) {
            $this->canvas = CanvasFactory::get_instance($this, $this->paperSize, $this->paperOrientation);
     ...
     *
     * @param string|float[] $size 'letter', 'legal', 'A4', etc. {@link PublishPress\Dompdf\Adapter\CPDF::$PAPER_SIZES}
    ...
    public function setPaper($size, string $orientation = "portrait"): self
    {
        ...
        if (
            \PublishPress\Dompdf\Helpers::lengthEqual($current_size[2], $new_size[2]) === false ||
            \PublishPress\Dompdf\Helpers::lengthEqual($current_size[3], $new_size[3]) === false
        ) {
            $this->canvas = CanvasFactory::get_instance($this, $this->paperSize, $this->paperOrientation);
        }
        return $this;
    ...
    /**
     * Return the underlying Canvas instance (e.g. PublishPress\Dompdf\Adapter\CPDF, PublishPress\Dompdf\Adapter\GD)
     *
     * @return Canvas
     */
