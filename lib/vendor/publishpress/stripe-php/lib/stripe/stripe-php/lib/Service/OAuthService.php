...
class OAuthService extends AbstractService
{
     ...
     * @param array|\PublishPress\Stripe\Util\RequestOptions $opts the special modifiers of the request
     *
     * @return \PublishPress\Stripe\StripeObject the object returned by Stripe's Connect API
     */
    ...
    public function authorizeUrl($params = null, $opts = null)
    {
        ...
        $query = \PublishPress\Stripe\Util\Util::encodeParameters($params);
     ...
     * @param null|array $opts
     *
     * @return \PublishPress\Stripe\StripeObject object containing the response from the API
     *
     * @throws \PublishPress\Stripe\Exception\OAuth\OAuthErrorException if the request fails
     */
    ...
    private function _getClientId($params = null)
    {
        ...
        if (null === $clientId) {
            $msg = 'No client_id provided. (HINT: set your client_id using '
              . '`new \PublishPress\Stripe\StripeClient([clientId => <CLIENT-ID>
            ...
            throw new \PublishPress\Stripe\Exception\AuthenticationException($msg);
        }
     ...
     * @param array|\PublishPress\Stripe\Util\RequestOptions $opts the special modifiers of the request
     *
     * @return \PublishPress\Stripe\Util\RequestOptions
     *
     * @throws \PublishPress\Stripe\Exception\InvalidArgumentException
     */
    private function _parseOpts($opts)
    {
        if (\is_array($opts)) {
            if (\array_key_exists('connect_base', $opts)) {
                ...
                // \PublishPress\Stripe\Service\OAuthService from \PublishPress\Stripe\OAuth, where `connect_base`
                // was the name of the parameter that behaves as `api_base` does here.
                throw new \PublishPress\Stripe\Exception\InvalidArgumentException('Use `api_base`, not `connect_base`');
            }
        }

        return \PublishPress\Stripe\Util\RequestOptions::parse($opts);
    }

    /**
     * @param \PublishPress\Stripe\Util\RequestOptions $opts
     *
     * @return string
     */
