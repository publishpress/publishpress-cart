...
use PublishPress\Stripe\Events\UnknownEventNotification;
use PublishPress\Stripe\Reason;
use PublishPress\Stripe\RelatedObject;
use PublishPress\Stripe\Util\EventNotificationTypes;
 ...
 * @property null|\PublishPress\Stripe\StripeContext        $context  Authentication context needed to fetch the event or related object.
...
abstract class EventNotification
{
     ...
     * @param \PublishPress\Stripe\StripeClient $client a StripeClient instance that this can use to make requests
     */
    public function __construct($json, $client)
    {
        ...
        if (\array_key_exists('context', $json) && null !== $json['context']) {
            $this->context = \PublishPress\Stripe\StripeContext::parse($json['context']);
        }
     ...
     * should use \PublishPress\Stripe\BaseStripeClient::parseEventNotification instead for
    ...
    protected function fetchRelatedObject()
    {
        ...
        return $this->client->deserialize($response->body, \PublishPress\Stripe\Util\Util::getApiMode($this->related_object->url));
    }
}
