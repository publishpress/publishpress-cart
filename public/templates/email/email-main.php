...
$image_url = get_option('_ppcart_company_logo');
$company_name = get_option('_ppcart_company_name');
$company_address = get_option('_ppcart_company_address');
$footer_text = get_option('_ppcart_email_footer_text');
$atts = $args;

$is_preview = filter_input(INPUT_GET, 'ppcart-preview', FILTER_SANITIZE_FULL_SPECIAL_CHARS);
$should_render_order_table = (
    ($atts['type'] == 'confirmation' || $atts['type'] == 'renewal' || $atts['type'] == 'pending')
    && (! function_exists('ppcart_email_type_uses_editable_order_details') || ! ppcart_email_type_uses_editable_order_details($atts['type']))
);

if ($should_render_order_table) {
    add_action('ppcart_before_email_footer', 'ppcart_do_order_table', 10, 2);
}
...
<?php // phpcs:ignore WordPress.WP.EnqueuedResources.NonEnqueuedStylesheet -- Email templates must include font links in the rendered email HTML.?>
</head>
...
<table width="100%" cellpadding="0" cellspacing="0" border="0">
  <tr>
    <td style="padding-right: 0px;padding-left: 0px;" align="center">
      ...
      <?php if ($image_url) : ?>
      <img align="center" border="0" src="<?php echo esc_url($image_url); ?>" alt="<?php echo esc_attr($company_name); ?>" title="<?php echo esc_attr($company_name); ?>" style="outline: none;text-decoration: none;-ms-interpolation-mode: bicubic;clear: both;display: inline-block !important;border: none;height: auto;float: none;width: 100%;max-width: 221px;" width="221"/>
      <?php endif; ?>
...
<?php do_action('ppcart_before_email_footer', $atts['type'], $atts['order_info']); ?>
...
<?php
if ($should_render_order_table) {
    remove_action('ppcart_before_email_footer', 'ppcart_do_order_table', 10);
}
?>
