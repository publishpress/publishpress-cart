...
test('EM-031 Order Refunded — disabled does not send', { tag: ["@admin","@email"] }, async ({ page, request }) => {
  ...
  const recipient = uniqueRecipient('refunded-disabled');

  await setEmailEnabled(page, config, 'refunded', false);
