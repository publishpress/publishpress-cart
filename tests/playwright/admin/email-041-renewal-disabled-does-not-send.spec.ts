...
test('EM-041 Subscription Renewal — disabled does not send', { tag: ["@admin","@email"] }, async ({ page, request }) => {
  ...
  const recipient = uniqueRecipient('renewal-disabled');

  await setEmailEnabled(page, config, 'renewal', false);
