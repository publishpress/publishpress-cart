...
const testCase = adminTestsById.get('AD-201');

if (!testCase) {
  throw new Error('Missing admin test definition: AD-201');
}

test('AD-201 Create Product With Payment Plan', { tag: ["@admin","@admin-workflow"] }, async ({ page }) => {
