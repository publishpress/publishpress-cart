...
const testCase = adminTestsById.get('AD-205');

if (!testCase) {
  throw new Error('Missing admin test definition: AD-205');
}

test('AD-205 Product Category Search Shows Created Term', { tag: ["@admin","@admin-workflow"] }, async ({ page }) => {
