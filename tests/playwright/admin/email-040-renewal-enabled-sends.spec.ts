...
test('EM-040 Subscription Renewal — enabled sends', { tag: ["@admin","@email"] }, async ({ page, request }) => {
  ...
  // Despite the name, `renewal` is not one of the statuses ppcart_notification_send()
  // builds a PPCart_Subscription for, so the order fixture is the right target.
  const descriptor = EMAIL_TYPES.renewal;
  const recipient = uniqueRecipient('renewal-enabled');

  await setEmailEnabled(page, config, 'renewal', true);
